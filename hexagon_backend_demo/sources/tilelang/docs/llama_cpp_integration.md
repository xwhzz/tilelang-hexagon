...
> **2026-07-16 update:** the qcm/scale cache and monolithic
> `tl_hexagon_hmx_gemm` path described below is now historical. The current Q4
> path consumes ggml's native 576-byte tiles, uses a native HVX dequant tile atom,
> maps its result directly into a `T.Layout`-defined weight Crouton, and emits
> explicit HMX state/instruction atoms. The staged schedule borrows ordered 2D
> DMA and synchronous parallel dispatch from ggml. See
> [`examples/hexagon/llama_cpp_integration/README.md`](../examples/hexagon/llama_cpp_integration/README.md)
> and the checked-in `kernel_qmatmul_hmx_{atoms,staged}_*.manifest.json` files. Sections
> that mention `kernel_qmatmul_compact`, qcm/sc repacking, or `T.gemm` are retained
> only as a record of the earlier proof.

### Current low-level boundary

The current Q4 reference deliberately stops below `T.gemm`:

1. The embedding runtime owns the HMX session, VTCM allocation, ordered DMA
   queue, and existing six-worker pool. `tl_op_ctx` only lends narrow `push_1d`,
   `push_2d`, `pop`, and synchronous `parallel.run` callbacks; an embedded kernel
   never creates threads or acquires a second device session.
2. TileLang owns Crouton `T.Layout` objects and the explicit
   `acquire/load_bias/clear/mma/convert/store/release` protocol.  Accumulator,
   convert, and bias buffers are dependency-only TIR tokens for implicit HMX
   state.
3. `Q4HMXIntrinEmitter.dequant_tile` exposes the hardware-sized dequant boundary:
   one native 576-byte Q4_0 tile becomes one 32x32 FP16 weight Crouton. Its v79
   implementation in `qmatmul.h` visibly reconstructs FP16 scales, extracts low
   and high nibbles, subtracts 8, multiplies, interleaves, and stores with HVX.
   Four independent register chains are interleaved for instruction-level
   parallelism. There is no row-major FP16 intermediate or `pack_b`.
4. Activation, weight, and result Croutons are 2 KB aligned; bias storage is
   256-byte aligned.  Activation and result instructions share one spatial-mask
   encoding.  FP32 boundaries convert in 32-lane chunks to preserve the HVX lane
   ABI.
5. The fast `M=32,N=256` schedule packs A once, uses 2D DMA to pad each raw row
   from 576 to 640 bytes, dequantizes disjoint linear tile ranges on the borrowed
   workers, and then runs the explicit HMX stage. A, B, C, bias, and two raw
   stages fit in the caller's 8 MB VTCM even at K=8192. Larger M values use the
   checked legacy 32-row loop rather than calling a fixed-M staged kernel.

The checked-in kernels pass standalone v79 device checks with relative errors
below `5e-4`, and deterministic llama.cpp generation matches stock exactly for
the tested prompt. On LFM2-1.2B Q4_0, repeated `llama-bench -p 32` measurements
were `616.78 +/- 29.77 tok/s` for the staged TileLang skel and
`604.36 +/- 36.28 tok/s` for stock. A final rebuilt skel measured
`643.00 +/- 6.53 tok/s`. The variance overlaps, so the defensible conclusion is
performance parity, not a proven speedup. The corresponding 2048x8192 operator
fell from about 5.02 ms in the serial baseline to 0.375-0.392 ms, matching stock's
0.385-0.408 ms range. The option still defaults to `OFF` because shape coverage
and production hardening are incomplete.

The previous compile failure was not caused by Q4 semantics or HMX code size.
A 64-lane nibble expression became `<64 x i32>` in LLVM and made Hexagon
SelectionDAG exceed 1 GB. Splitting low/high integer arithmetic into independent
32-lane vectors and interleaving only after FP16 conversion made codegen bounded.
On-device profiling then exposed serial dequantization as the real bottleneck;
the native tile atom plus borrowed workers removes that bottleneck without a
persistent device-memory cache.
