...
#include <AEEStdErr.h>
#include <type_traits>
#include "tl_matmul_kernel.h"   // QAIC-generated handler prototypes

#include <tl_templates/hexagon/vtcm.h>
...
AEEResult tl_matmul_kernel_open(const char* uri, remote_handle64* h) { (void)uri; *h = 0; tl_hmx_session_init(); if (!tl_hmx_session_ok()) return AEE_EFAILED; return AEE_SUCCESS; }
AEEResult tl_matmul_kernel_close(remote_handle64 h) { (void)h; tl_hmx_session_deinit(); return AEE_SUCCESS; }

AEEResult tl_matmul_kernel_run(remote_handle64 _h, const unsigned char* p0, int p0Len, const unsigned char* p1, int p1Len, unsigned char* p2, int p2Len) {
  return tl_hexagon_forward_kernel_status([&]() { return matmul_kernel((__fp16*)p0, (__fp16*)p1, (__fp16*)p2); });
}
