...
extern "C"
#endif
int32_t tl_q4_hmx_m32_n512_k2048_kernel(float* A, uint8_t* W, float* C) {
  int bx = 0;
  for (bx = 0; bx < 1; ++bx) {
...
extern "C"
#endif
int32_t tl_q4_hmx_m32_n512_k2048_embedded(void* vtcm_base, unsigned int vtcm_size, float* activation, uint8_t* weight, float* output) {
  if (tl_bridge_enter(vtcm_base, vtcm_size) != 0) return -1;
  int32_t rc = tl_q4_hmx_m32_n512_k2048_kernel(activation, weight, output);
  tl_bridge_exit();
  return rc;
}
