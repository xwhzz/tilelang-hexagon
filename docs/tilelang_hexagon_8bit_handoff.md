...
- **Tier 2 · `HMXIntrinEmitter`**(`tilelang/hexagon/hmx_intrin.py`):instruction-atom,用户**手驱 K-loop**
  (`begin / pack_a / clear / pack_b / mma / store / end`),让 dequant 交织在 MAC 之间(K-streaming)。
  CUDA `TensorCoreIntrinEmitter` 的 Hexagon 对应。**诚实的分歧**:HMX 没有可寻址累加器片段 —— 一个
  隐形的 per-core 累加器,所以 `clear/mma/store` 不 thread `C_local`。
...
### 4b. llama.cpp 对接机制("Mode B",correctness PoC,prefill parity)[源码]
...
4. **拦截点** [关键]:patch 在 `matmul-ops.c` 的 **`hmx_mm_2d_f32(...)` 顶部**插 `if (tl_dispatch(&octx)==0) return 0;`。
   门控:`m<=32 && weight==Q4_0 && ...` → **只拦 prefill 的 HMX 路径**。
...
- ⚠️ 是 **q4_0** 的,且是 **correctness PoC**(证明能正确跑,达 prefill parity,**没 beat**)。
